...
function leftClickHandler(e) { // Private method
 ...
 hideVisibleMenus();
  ...
  if (popUpMenuObj) {
    ...
    if ((state == "hidden" || state == "") && (showValue == 0 || showValue == 2) && (state=="" && document.forms[0].ImgContextMenuClick.value==1)) {
      activatePopUpMenu(e);
   }
...
function addMenuItem(menuItemObj) { // Public method
  if (menuItemObj.displayText == "-") {
	...
	if(itemElm.enabled)
	itemElm.itemClassName = "jsdomenuitem";
	else
    itemElm.itemClassName = "disable";
...
function activatePopUpMenuBy() { // Public method
 ...
 //  document.getElementById('Information').oncontextmenu = rightClickHandler;
 rightClickHandler();
  //  document.getElementById('A').onClick = rightClickHandler;		
